...
app.get('/', function(req, res){
  res.status(200).send("CenturyLink Coding Challenge");
});

var firstLevelFollowers = [];
var secondLevelFollowers = [];
var thirdLevelFollowers = [];
var userRepos = [];
var starGazers = [];
      ...
      request.get(options, function (error, response, body) {
        if (error) {
          ...
          for (var i = 0; i < jsonObj.length && i < 5; i++) {
            firstLevelFollowers.push(jsonObj[i]);
          }

          callback(null, firstLevelFollowers);
      });
    },
    function getSecondLevelFollowers(firstGithubFollowers, callback) {
        ...
        async.each(firstGithubFollowers, function(follower, callback) {
          ...
          request.get(options, function(error, response, body) {
              ...
              secondLevelFollowers = [];
              var jsonObj = JSON.parse(body);
              for (var i = 0; i < jsonObj.length && i < 5; i++) {
                follower.second_level_followers.push(jsonObj[i]);
                secondLevelFollowers.push(jsonObj[i]);
              }
              callback();
          });
          ...
          if (err) {
            ...
            callback(null, secondLevelFollowers);
          }
        });
    },
    function getThirdLevelFollowers(secondGithubFollowers, callback) {
        async.each(secondGithubFollowers, function(follower, callback) {
          ...
          request.get(options, function(error, response, body) {
              ...
              for (var i = 0; i < jsonObj.length && i < 5; i++) {
                ...
                thirdLevelFollowers.push(jsonObj[i]);
              }
